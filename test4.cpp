...
int main()
{
	...
	cv::Mat binImage = cv::imread("test4.png");
	...
	// 全局二值化
	int th = 160; 
	cv::threshold(binImage, binImage, th, 255, CV_THRESH_BINARY);
 
	cv::Mat labelImg;
    cv::cvtColor(binImage, binImage, CV_BGRA2GRAY);
    // cv::cvtColor(binImage, binImage, CV_GRAY2BGR);
    cv::imshow("aa",binImage);
	...
	cv::imshow("labelImg", ~grayImg);
    cv::imwrite("labeImg.jpg", ~grayImg);
